...
TEST_CASE("WARP menu marker observes reduced scene binds without changing them", "[native_ui]") {
    ...
    REQUIRE_FALSE(redirect.compatibilityFault());
    auto inventoryDesc=desc;
    inventoryDesc.Format=DXGI_FORMAT_R16G16_FLOAT;
    ComPtr<ID3D11Texture2D> inventoryAuxiliary;
    REQUIRE(SUCCEEDED(device->CreateTexture2D(&inventoryDesc,nullptr,
        &inventoryAuxiliary)));
    ComPtr<ID3D11RenderTargetView> inventoryView;
    REQUIRE(SUCCEEDED(device->CreateRenderTargetView(inventoryAuxiliary.Get(),
        nullptr,&inventoryView)));
    reducedTargets[1]=inventoryView.Get();
    redirect.onOMSetRenderTargets(context.Get(),2,reducedTargets.data(),depthView.Get());
    ...
    REQUIRE(fault.unknownTargetFormat==DXGI_FORMAT_R16G16_FLOAT);
    REQUIRE(fault.depthId==fault.expectedDepthId);
    REQUIRE(fault.learnedAuxiliary);
    ...
    REQUIRE(route.begin(8,1,GetCurrentThreadId()));
    REQUIRE(SUCCEEDED(redirect.prepareObservedCompanions()));
    REQUIRE_FALSE(redirect.hasUnpreparedAuxiliary());
    REQUIRE_FALSE(redirect.resumeLatePassRouting(8,false));
    REQUIRE(route.startProcessing(8,1));
    REQUIRE(SUCCEEDED(redirect.commitPublishedUi(8)));
    REQUIRE(route.closePublishedFrame(8,1));
    REQUIRE(route.begin(9,1,GetCurrentThreadId()));
    REQUIRE(redirect.resumeLatePassRouting(9,false));
    ...
    redirect.onOMSetRenderTargets(context.Get(),2,reducedTargets.data(),depthView.Get());
    REQUIRE_FALSE(redirect.compatibilityFault());
    nativeTargets[0].Reset();nativeTargets[1].Reset();boundDepth.Reset();
    rawTargets={};
    context->OMGetRenderTargets(2,rawTargets.data(),boundDepth.GetAddressOf());
    for(std::size_t i=0;i<nativeTargets.size();++i)
        nativeTargets[i].Attach(rawTargets[i]);
    REQUIRE(viewExtent(nativeTargets[1].Get()).width==display.width);
    REQUIRE(viewExtent(nativeTargets[1].Get()).height==display.height);
    reducedTargets[1]=auxiliaryView.Get();
    redirect.onOMSetRenderTargets(context.Get(),2,reducedTargets.data(),depthView.Get());
    REQUIRE_FALSE(redirect.compatibilityFault());
    reducedTargets[1]=secondAuxiliaryView.Get();
    ...
    redirect.onOMSetRenderTargets(context.Get(),2,reducedTargets.data(),depthView.Get());
    REQUIRE(redirect.compatibilityFault());
    REQUIRE_FALSE(redirect.compatibilityFaultInfo().learnedAuxiliary);
