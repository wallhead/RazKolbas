...
TEST_CASE("WARP menu marker observes reduced scene binds without changing them", "[native_ui]") {
    ...
    const auto fault=redirect.compatibilityFaultInfo();
    REQUIRE(fault.unknownTargetSlot==1);
    REQUIRE(fault.unknownTargetMips==1);
    REQUIRE(fault.unknownTargetFormat==desc.Format);
    REQUIRE(fault.depthId==fault.expectedDepthId);
    REQUIRE(fault.unknownTargetBindFlags==desc.BindFlags);
    ComPtr<ID3D11ShaderResourceView> unknownShaderView;
    REQUIRE(SUCCEEDED(device->CreateShaderResourceView(unknownReduced.Get(),
        nullptr,&unknownShaderView)));
    auto* unknownShaderRaw=unknownShaderView.Get();
    redirect.onPSSetShaderResources(context.Get(),5,1,&unknownShaderRaw);
    const auto afterRead=redirect.compatibilityFaultInfo();
    REQUIRE(afterRead.unknownTargetSrvReads==1);
    REQUIRE(afterRead.unknownTargetFirstSrvSlot==5);
    redirect.suspendLatePassRouting(7);
    REQUIRE_FALSE(redirect.compatibilityFault());
    REQUIRE_FALSE(redirect.latePassRoutingAvailable());
    REQUIRE(redirect.hasUnpreparedAuxiliary());
    REQUIRE(SUCCEEDED(redirect.prepareObservedCompanions()));
    REQUIRE_FALSE(redirect.hasUnpreparedAuxiliary());
    REQUIRE(redirect.companionsReady());
    REQUIRE(route.closePublishedFrame(7,1));
    REQUIRE(route.begin(9,1,GetCurrentThreadId()));
    REQUIRE(redirect.resumeLatePassRouting(9,true));
    REQUIRE(redirect.latePassRoutingAvailable());
    REQUIRE(route.startProcessing(9,1));
    REQUIRE(SUCCEEDED(redirect.commitPublishedUi(9)));
    reducedTargets[1]=unknownReducedView.Get();
    redirect.onOMSetRenderTargets(context.Get(),2,reducedTargets.data(),depthView.Get());
    REQUIRE_FALSE(redirect.compatibilityFault());
    std::array<ID3D11RenderTargetView*,2> sampledTargets{};
    context->OMGetRenderTargets(2,sampledTargets.data(),nullptr);
    ComPtr<ID3D11RenderTargetView> sampledSceneBound,sampledAuxiliaryBound;
    sampledSceneBound.Attach(sampledTargets[0]);
    sampledAuxiliaryBound.Attach(sampledTargets[1]);
    REQUIRE(viewExtent(sampledAuxiliaryBound.Get()).width==display.width);
    REQUIRE(identity(viewResource(sampledAuxiliaryBound.Get()).Get()).Get()!=
        identity(unknownReduced.Get()).Get());
    redirect.onOMSetRenderTargets(context.Get(),1,&sceneView,depthView.Get());
    redirect.onPSSetShaderResources(context.Get(),5,1,&unknownShaderRaw);
    ComPtr<ID3D11ShaderResourceView> sampledAuxiliarySrv;
    context->PSGetShaderResources(5,1,&sampledAuxiliarySrv);
    REQUIRE(sampledAuxiliarySrv!=nullptr);
    REQUIRE(viewExtent(sampledAuxiliarySrv.Get()).width==display.width);
    REQUIRE(identity(viewResource(sampledAuxiliarySrv.Get()).Get()).Get()==
        identity(viewResource(sampledAuxiliaryBound.Get()).Get()).Get());
    desc.MipLevels=2;
    ComPtr<ID3D11Texture2D> unsupportedReduced;
    REQUIRE(SUCCEEDED(device->CreateTexture2D(&desc,nullptr,&unsupportedReduced)));
    ComPtr<ID3D11RenderTargetView> unsupportedView;
    REQUIRE(SUCCEEDED(device->CreateRenderTargetView(unsupportedReduced.Get(),
        &unknownViewDesc,&unsupportedView)));
    reducedTargets[1]=unsupportedView.Get();
    redirect.onOMSetRenderTargets(context.Get(),2,reducedTargets.data(),depthView.Get());
    REQUIRE(redirect.compatibilityFault());
    redirect.suspendLatePassRouting(9);
    REQUIRE(route.closePublishedFrame(9,1));
    REQUIRE(route.begin(512,1,GetCurrentThreadId()));
    REQUIRE_FALSE(redirect.resumeLatePassRouting(512,true));
    REQUIRE_FALSE(redirect.latePassRoutingAvailable());
    route.suspend();
