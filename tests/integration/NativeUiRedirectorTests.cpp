...
TEST_CASE("WARP menu marker observes reduced scene binds without changing them", "[native_ui]") {
    ...
    REQUIRE(fault.unknownTargetMips==2);
    ...
    REQUIRE_FALSE(redirect.resumeLatePassRouting(126,true));
    REQUIRE(route.closePublishedFrame(7,1));
    REQUIRE(route.begin(127,1,GetCurrentThreadId()));
    REQUIRE(redirect.beginFaultTrace(127));
    redirect.onOMSetRenderTargets(context.Get(),2,reducedTargets.data(),depthView.Get());
    redirect.onRSSetViewports(context.Get(),1,&reducedViewport);
    redirect.onPSSetShaderResources(context.Get(),5,1,&unknownShaderRaw);
    const auto trace=redirect.finishFaultTrace(127);
    REQUIRE(trace.has_value());
    REQUIRE(trace->frame==127);
    REQUIRE(trace->count==3);
    REQUIRE(trace->dropped==0);
    REQUIRE(trace->events[0].kind==rk::UiFaultTraceKind::RenderTargets);
    REQUIRE(trace->events[0].targets[1].id==
        reinterpret_cast<std::uintptr_t>(identity(unknownReduced.Get()).Get()));
    REQUIRE(trace->events[1].kind==rk::UiFaultTraceKind::Viewport);
    ...
    REQUIRE(route.startProcessing(127,1));
    REQUIRE(SUCCEEDED(redirect.commitPublishedUi(127)));
    reducedTargets[1]=auxiliaryView.Get();
    redirect.onOMSetRenderTargets(context.Get(),2,reducedTargets.data(),depthView.Get());
    REQUIRE_FALSE(redirect.compatibilityFault());
    reducedTargets[1]=unknownReducedView.Get();
    redirect.onOMSetRenderTargets(context.Get(),2,reducedTargets.data(),depthView.Get());
    REQUIRE(redirect.compatibilityFault());
    redirect.suspendLatePassRouting(127);
    REQUIRE(route.closePublishedFrame(127,1));
