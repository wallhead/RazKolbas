...
| P1 | Fast-FP16 NR now produces nontrivial FP16 and RGBA8 output in the isolated RTX 4080 SUPER probe, and the full D3D11-copy/D3D12-evaluate/D3D11-copyback bridge passed 30 frames. Direct D3D11 initialization returned `0xBAD00001`, so the game candidate uses D3D12 interop. Skyrim temporal guide semantics and image quality remain unverified, and the current bridge waits synchronously on every frame. The modified runtime has a retained NVIDIA signer but `HashMismatch`; only exact SHA-256 `91ea...` is accepted. | Run the installed 0.1.80 candidate through startup and save load. Confirm NR submission logs, visual stability, ENB/ReShade/native-UI preservation and frame time. If guide semantics are correct, replace the synchronous single-frame wait with a fenced context ring before calling NR performance-ready. |
