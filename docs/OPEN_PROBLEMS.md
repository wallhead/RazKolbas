...
| P1 | The 0.1.67 user-started test still rotated the camera because it used the 1.5.97 `ControlMap::ignoreKeyboardMouse` offset `+0x121` on 1.6.1170. Installed candidate 0.1.68 selects the verified AE offset `+0x129` while preserving `+0x121` for 1.5.97; Debug and Release pass all 39 tests. | Verify slider dragging, checkbox/combo selection, no camera movement, focus-loss restoration, INI persistence and the next-launch quality/model transition in one user-started run. |
