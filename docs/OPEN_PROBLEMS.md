...
| P1 | Fast-FP16 NR produces nontrivial output in the isolated probe and the 30-frame D3D11/D3D12 bridge passes live control changes. The 0.1.81 NativeAA run reached the NR preprocessor but disabled NR on frame 1 because native preparation supplied R24 depth to the R32-only bridge; DLAA continued fail-open. Source 0.1.82 normalizes and refreshes full-size depth through the existing conversion shader. Skyrim temporal quality remains unverified, and the bridge still waits synchronously per frame. The modified runtime has retained NVIDIA signer metadata but `HashMismatch`; only exact SHA-256 `91ea...` is accepted. | Install 0.1.82 after Skyrim exits. In NativeAA, confirm NR submissions continue beyond frame 1, change Intensity once, and inspect motion, ENB/ReShade and native UI. Then repeat DLSS Quality. If guide semantics are correct, replace the synchronous single-frame wait with a fenced context ring before calling NR performance-ready. |
