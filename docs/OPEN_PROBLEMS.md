...
| P1 | The 0.1.68 user-started test proved the corrected `ControlMap +0x129` byte changes, but camera movement still reaches gameplay. Source 0.1.69 instead gates the verified 1.6.1170 input-dispatch CALL, forwarding a null event list only while the menu is visible and preserving the existing SmartTalk hook chain. Debug and Release pass all 40 tests. | Install 0.1.69, then verify slider dragging, checkbox/combo selection, no camera movement, SmartTalk behavior after closing the menu, focus-loss restoration, INI persistence and the next-launch quality/model transition. |
