...
| P1 | Fast-FP16 NR now passes sustained actual-game NativeAA operation: 0.1.82 completed at least 13,800 NR-before-DLAA submissions while live controls reset history, DLAA reached 14,400 with no skips, and Present reached 21,000 with zero failures. The user reports that it appears to work. Objective motion/ghosting comparison, GPU time and reduced DLSS Quality NR remain unverified; the bridge still waits synchronously per frame. The modified runtime has retained NVIDIA signer metadata but `HashMismatch`; only exact SHA-256 `91ea...` is accepted. | Repeat with DLSS Quality and inspect motion, ENB/ReShade and native UI. Compare NR off/on at fixed camera and movement, then measure GPU time. If guide semantics remain correct, replace the synchronous single-frame wait with a fenced context ring before calling NR performance-ready. |
