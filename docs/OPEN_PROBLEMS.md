...
deferred `GRenderer::EndFrame` stencil repair. The same run exposes periodic
resource-fill flicker while publication oscillates between menu and
pre-Present boundaries. Source 0.1.76 latches the verified menu boundary and
retires the disproved scissor mutation. Actual-game verification is next.
...
|---|---|---|
| P0 | Installed 0.1.75 removes the two isolated actor-level fragments. Resource fills then periodically turn black while the frames remain. The run log shows single low-diversity depth samples repeatedly move composition from menu-boundary `mode=2` to pre-Present `mode=3` and back. | Install 0.1.76 and verify health, stamina and magicka fills remain stable through the same scene. Confirm the two former fragments stay absent and ENB remains correct. |
...
command-driven game run. The next evidence is one normal user-started 0.1.76
save-load run at the same scene; boundary latching is automatic. The
