...
| P1 | The End menu now reports the working owned DLSS route and provides live post-sharpening plus persisted quality/model controls. Mouse input is polled directly and ImGui draws its cursor above the overlay, but interaction with the new widgets has not run in game. | Verify slider dragging, checkbox/combo selection, INI persistence and the next-launch quality/model transition in one user-started run. |
