...
| P1 | The 0.1.66 End menu and widgets render, but Skyrim also consumes mouse movement and rotates the camera. Source 0.1.67 now gates the supported-runtime `ControlMap::ignoreKeyboardMouse` byte while the menu is visible and focused, preserving and restoring the prior value. | Install 0.1.67, then verify slider dragging, checkbox/combo selection, no camera movement, focus-loss restoration, INI persistence and the next-launch quality/model transition in one user-started run. |
