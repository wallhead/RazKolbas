...
original failure, tests post-init exception/retirement failures, then positive
evaluation with a deterministic known image and readback validation.
"""
import argparse
import os
...
def main():
    ...
    cases = [
        ...
        ("missing-release", feature, "omit_one_release", 10,
         ...
         ["GPU_OUTPUT=NONTRIVIAL", "PATCH_RESTORED"]),
        ("evaluate", feature, None, 0,
         ["RAW_FEATURE_CREATE=0x1; handle_nonnull=1", "RAW_FEATURE_EVALUATE=0x1",
          "FEATURE_CREATION_RETIREMENT=PASS; EVALUATE=PASS; GPU_OUTPUT=NONTRIVIAL",
          "finite=921600/921600", "changed_from_sentinel=921600",
        ...
        ("rgba8-evaluate", feature, None, 0,
         ["EVALUATION_COLOR_FORMAT=R8G8B8A8_UNORM", "RAW_FEATURE_EVALUATE=0x1",
          "FEATURE_CREATION_RETIREMENT=PASS; EVALUATE=PASS; GPU_OUTPUT=NONTRIVIAL",
          "RAW_SHUTDOWN_RESULT=0x1", "PATCH_RESTORED=nr.caller-name.experiment"],
         ["INJECTED_", "FEATURE_EXPERIMENT_STOP", "FEATURE_EXPERIMENT_EXCEPTION"]),
    ]
    ...
    for name, extra, fault, expected_exit, required, forbidden in cases:
        ...
        environment.pop("RAZKOLBAS_NR_PROBE_FORMAT", None)
        if fault:
            environment["RAZKOLBAS_NR_PROBE_FAULT"] = fault
        if name == "rgba8-evaluate":
            environment["RAZKOLBAS_NR_PROBE_FORMAT"] = "rgba8"
        try:
