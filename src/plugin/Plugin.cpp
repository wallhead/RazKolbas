...
extern "C" __declspec(dllexport) constinit SKSE::PluginVersionData SKSEPlugin_Version = [] {
    ...
    metadata.PluginVersion({0, 1, 14, 0});
...
extern "C" __declspec(dllexport) bool SKSEPlugin_Load(const SKSE::detail::SKSEInterface* skse) noexcept {
    ...
    try {
        ...
        spdlog::info("RazKolbas 0.1.14 native host bootstrap {}; vendor providers inactive until guarded offscreen probe", started ? "ready" : "failed");
        if (started) {
