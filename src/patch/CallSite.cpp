...
Result<bool> verifySkyrim1170MenuDisplayCallAbi(
    ...
    // RVA fa51b4: four arguments for e441c0, then the direct CALL. The
    // continuation loads the current IMenu and invokes PostDisplay at +0x30.
    constexpr std::array<std::uint8_t,38> expected{
        ...
        0xff,0x50,0x30,0x48};
    // e441c0 consumes EDX, R8D and R9B in its first 66 bytes. RCX is
    // intentionally preserved by the proxy even though this target replaces
    // it before its first call.
    constexpr std::array<std::uint8_t,66> expectedTarget{
    ...
    if(caller.size()<expected.size()||originalTarget.size()<expectedTarget.size()||
       !std::equal(expected.begin(),expected.end(),caller.begin())||
